 ...
 */
Energy2026::Energy2026(State* pState)  : m_Box(pState->GetMesh()->Link2ReferenceBox()) {
    
    m_pState = pState;
}
Energy2026::~Energy2026() {
    
}
inline double Energy2026::SingleVertexEnergy(vertex* __restrict v)
{
    double energy = 0.0;

    // Cache state and sub-objects
    auto* state      = m_pState;
    auto* extField   = state->GetExternalFieldOnInclusions();
    auto* adhesion   = state->GetVertexAdhesionToSubstrate();
    auto* stretching = state->GetAbstractLocalStretching();

    energy += extField->GetCouplingEnergy(v);
    energy += adhesion->GetCouplingEnergy(v);
    energy += stretching->Energy(v);

    // Better branch structure
    switch (v->m_VertexType)
    {
        case 0:
            energy += SurfVertexBendingAndStretchingEnergy(v);
            break;

        case 1:
            energy += EdgeVertexBendingAndStretchingEnergy(v);
            break;
    ...
    v->UpdateEnergy(energy);
    return energy;
}
inline double Energy2026::SurfVertexBendingAndStretchingEnergy(vertex* __restrict p_vertex) {
...
inline double Energy2026::EdgeVertexBendingAndStretchingEnergy(vertex* __restrict p_vertex)
{
...
double Energy2026::CalculateAllLocalEnergy()
{
    double E = 0.0;
...
double Energy2026::TwoInclusionsInteractionEnergy(links * p_edge) {
...
//--> now use the type if to get the interaction parameters of this pars
    ...
    switch (FunctionType) {
        ...
        case 6: {
            e_int = Interaction_6(ff[0], ff[1], ff[2],ff[3],  p_edge);

            break;
        }
        case 7: {
            e_int = Interaction_7(ff[0], ff[1], ff[2],ff[3], ff[4], p_edge);
...
double Energy2026::InteractionFunction(double N, double A, double B, double theta) {
...
double Energy2026::InteractionFunctionFull(double N, double A, double B, double theta, links* pl) {
...
double Energy2026::InteractionFive(double N, double A, double B, double C,  links* p_link){
...
double Energy2026::Interaction_6(double N, double A, double B, double C,  links* p_link){
    ...
    e = -A + B * e + C * cos2T_1 * (edge_size-1.0);

return e;
    
}
double Energy2026::Interaction_7(double N, double A, double B, double C, double l_0,  links* p_link){
    ...
    e = -A + B * e + C * cos2T_1 * (edge_size-l_0) * (edge_size-l_0);

return e;
    
}
double Energy2026::Filament_int(double A, double B, double C, vertex* p_v1, vertex* p_v2) {
...
double Energy2026::F2(vertex *v1, vertex *v2, std::vector<double> var)
{
...
double Energy2026::F10(vertex *v1, vertex *v2, std::vector<double> var)
{
...
double Energy2026::F11(vertex *v1, vertex *v2, std::vector<double> var)
{
...
double Energy2026::TwoVectorFieldInteractionEnergy(int vf_layer, links * p_edge) {
...
double Energy2026::AngleDiff_ParallelTransport(Vec3D &d1, Vec3D &d2, links* p_link) {
    /*
...
double Energy2026::Geo_Theta(vertex *v1, vertex *v2) {
    /*
     * @brief Calculate the angle between two vectors after parallel transport along the geodesic direction.
     *
     * This function computes the angle between two vectors after parallel transport along the geodesic direction
...
double Energy2026::CalculateVectorFieldMembraneBindingEnergy(VectorField* p_vf, vertex *p_vertex){
...
double Energy2026::CalculateVectorFieldMembraneBindingEnergy(vertex *p_vertex){
...
std::string Energy2026::CurrentState(){
...
AbstractEnergy* Create_ReG(
        ...
        State* state)
    {
        ...
        return new Energy2026(state);
    }

    const bool registered = []()
    {
        FactoryEnergyFunctions::Instance().Register(
        Energy2026::GetDefaultReadName(),
            &Create_ReG
        );
        return true;
    }();
