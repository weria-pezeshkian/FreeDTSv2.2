...
/*
 * @brief Energy calculation based on FreeDTS1.1 force field optmised version 2026.
 *
 ...
 */
class State;
class Energy2026 : public AbstractEnergy {
public:
    Energy2026(State* pState);
	 ~Energy2026();

public:
    inline std::string GetDerivedDefaultReadName() {return "Energy2026";}
    inline static std::string GetDefaultReadName() {return "Energy2026";}
    ...
    inline double SingleVertexEnergy(vertex* __restrict p);
private:
    inline double SurfVertexBendingAndStretchingEnergy(vertex* __restrict p);
    inline double EdgeVertexBendingAndStretchingEnergy(vertex* __restrict p);
