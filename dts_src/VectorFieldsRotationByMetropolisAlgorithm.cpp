...
bool VectorFieldsRotationByMetropolisAlgorithm::EvolveOneStep(int step){
    ...
    for (int layer = 0; layer< m_No_VectorFields_Per_V; layer++) {
        for (int i = 0; i< no_steps; i++) {
            ...
            if(RotationMove(layer, m_pActiveV[r_v_id], thermal, m_DR*dx, m_DR*dy)){
                m_AcceptedMoves++;
            }
...
bool VectorFieldsRotationByMetropolisAlgorithm::RotationMove(int layer, vertex *p_vertex, double dx, double dy, double temp){
    ...
    if(p_vertex->GetVertexType() == 1){
     ...
     old_energy += p_vf->GetMembraneBindingEnergy(); // (*it)->GetBindingEnergy();
     p_vf->Copy_BindingEnergy();
...
//-- perfrom the move
    ...
    new_d.normalize();
    p_vf->UpdateLocalDirection(new_d);
    
    new_energy += p_vf->CalculateMembraneBindingEnergy(p_vertex);
